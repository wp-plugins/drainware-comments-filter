...
function drainware_spam_comments( $type = false, $page = 1, $per_page = 50 ) {
	global $wpdb;
...
function drainware_spam_totals() {
	global $wpdb;
...
function drainware_manage_page() {
	...
	$count = sprintf(__('drainware Spam (%s)'), drainware_spam_count());
	if ( isset( $submenu['edit-comments.php'] ) )
		add_submenu_page('edit-comments.php', __('drainware Spam'), $count, 'moderate_comments', 'drainware-admin', 'drainware_caught' );
	elseif ( function_exists('add_management_page') )
		add_management_page(__('drainware Spam'), $count, 'moderate_comments', 'drainware-admin', 'drainware_caught');
}

function drainware_caught() {
	global $wpdb, $comment, $drainware_caught, $drainware_nonce;

	drainware_recheck_queue();
	if (isset($_POST['submit']) && 'recover' == $_POST['action'] && ! empty($_POST['not_spam'])) {
		...
		foreach ($_POST['not_spam'] as $comment):
			...
			drainware_submit_nonspam_comment($comment);
			++$i;
		endforeach;
	...
	if ('delete' == $_POST['action']) {
		...
		if ( !empty( $comment_ids ) ) {
			...
			wp_cache_delete( 'drainware_spam_count', 'widget' );
		}
...
<?php
$count = get_option( 'drainware_spam_count' );
if ( $count ) {
?>
<p><?php printf(__('drainware has caught <strong>%1$s spam</strong> for you since you first installed it.'), number_format_i18n($count) ); ?></p>
<?php
}

$spam_count = drainware_spam_count();
...
<?php drainware_nonce_field($drainware_nonce) ?>
...
<h2><?php _e('Search'); ?></h2>
<?php } else { ?>
<?php echo '<p>'.__('These are the latest comments identified as spam by drainware. If you see any mistakes, simply mark the comment as "not spam" and drainware will learn from the submission. If you wish to recover a comment from spam, simply select the comment, and click Not Spam. After 15 days we clean out the junk for you.').'</p>'; ?>
<?php } ?>
<?php
if ( isset( $_POST['s'] ) ) {
	...
	$comments = drainware_spam_comments( $current_type, $page );
	$total = drainware_spam_count( $current_type );
	$totals = drainware_spam_totals();
?>
<ul class="drainware-tabs">
<li <?php if ( !isset( $_GET['ctype'] ) ) echo ' class="active"'; ?>><a href="edit-comments.php?page=drainware-admin"><?php _e('All'); ?></a></li>
<?php
foreach ( $totals as $type => $type_count ) {
	...
	echo "<li $extra><a href='edit-comments.php?page=drainware-admin&amp;ctype=$type'>$show ($type_count)</a></li>";
}
do_action( 'drainware_tabs' ); // so plugins can add more tabs easily
?>
</ul>
<?php
}
...
if ($comments) {
?>
<form method="post" action="<?php echo attribute_escape("$link?page=drainware-admin"); ?>" id="drainwaresearch">
...
<p><?php _e('Comments you de-spam will be submitted to drainware as mistakes so it can learn and get better.'); ?></p>
</form>
<?php
} else {
?>
...
add_action('admin_menu', 'drainware_manage_page');

function redirect_old_drainware_urls( ) {
	...
	if ( 8204 < $wp_db_version && ( 'edit-comments.php' == $script_name || 'edit.php' == $script_name ) && 'drainware-admin' == $page ) {
		$new_url = esc_url( 'edit-comments.php?comment_status=spam' );
		wp_redirect( $new_url, 301 );
		exit;
	}
}
add_action( 'admin_init', 'redirect_old_drainware_urls' );

// For WP <= 2.3.x
global $pagenow;

if ( 'moderation.php' == $pagenow ) {
	function drainware_recheck_button( $page ) {
		global $submenu;
		if ( isset( $submenu['edit-comments.php'] ) )
			$link = 'edit-comments.php';
		else
			$link = 'edit.php';
		$button = "<a href='$link?page=drainware-admin&amp;recheckqueue=true&amp;noheader=true' style='display: block; width: 100px; position: absolute; right: 7%; padding: 5px; font-size: 14px; text-decoration: underline; background: #fff; border: 1px solid #ccc;'>" . __('Recheck Queue for Spam') . "</a>";
	...
	if ( $wpdb->get_var( "SELECT COUNT(*) FROM $wpdb->comments WHERE comment_approved = '0'" ) )
		ob_start( 'drainware_recheck_button' );
}

// This option causes tons of FPs, was removed in 2.1
function drainware_kill_proxy_check( $option ) { return 0; }
add_filter('option_open_proxy_check', 'drainware_kill_proxy_check');
